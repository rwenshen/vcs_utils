...
class VCSTestBase(unittest.TestCase, ABC):
    ...
    def setUpClass(cls):
        if cls.testRoot.exists():
            ...
            if sys.version_info <= (3, 11):
                shutil.rmtree(cls.testRoot, onerror=del_rw)
            else:
    ...
    def vcsName(self) -> str:
        return self.__class__._vcsName
...
#    # common functions
#    def checkGetNewCommit(self, repo: Repo) -> Commit:
#        commit = repo.getNewCommit()
#        self.assertIsNotNone(commit)
#        if commit is not None:
#            self.assertTrue(commit.isWritable)
#            self.assertIsNotNone(commit.vcsData)
#            self.assertIsNotNone(commit.commitRef)
#            self.assertEqual(commit.repo, repo)
#            return commit
#        # impossible
#        return None # type: ignore
#    
#    def checkGetTopCommit(self, repo: Repo) -> Commit:
...
#        # new commit
#        commit = self.checkGetNewCommit(repo)
#        # empty commit
...
#        commit = self.checkGetTopCommit(repo)
